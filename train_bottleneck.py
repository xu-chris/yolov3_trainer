...
def _main():
    annotation_path = 'dist/annotations.txt'
    ...
    # Make log_dir folder
    folder_hash = str(sum(os.path.isdir(i) for i in os.listdir('logs')) + 1)
    ...
    model, bottleneck_model, last_layer_model = create_model(input_shape, anchors, num_classes,
            freeze_body=2, weights_path='source/darknet53_weights.h5') # make sure you know what you freeze
...
def create_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2,
            weights_path='source/yolo_weights.h5'):
